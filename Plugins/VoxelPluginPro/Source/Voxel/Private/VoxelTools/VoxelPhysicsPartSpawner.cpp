...
{
	const auto Data = Part.Data.ToSharedRef();
	...
	Transform.SetLocation(World->LocalToGlobal(Part.Position));
	...
	for (TFieldIterator<FProperty> It(AVoxelRuntimeActor::StaticClass(), EFieldIteratorFlags::ExcludeSuper); It; ++It)
	{
		auto* Property = *It;
	...
	// Voxel size will be applied by object scale
	NewWorld->CollisionPresets.COMNudge = Part.CenterOfMass;
	...
	FVoxelFloatingPart&& Part)
{
	...
	for (const auto& Voxel : Part.Voxels)
	{
		...
		auto* StaticMeshActor = GetWorld()->SpawnActor<AStaticMeshActor>(
			World->LocalToGlobal(Voxel.Position + Part.Position),
			Rotation);
	...
	FVoxelFloatingPart&& Part)
{
	...
	Result->Voxels = MoveTemp(Part.Voxels);
	return Result;
}

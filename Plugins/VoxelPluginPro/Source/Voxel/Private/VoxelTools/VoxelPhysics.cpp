...

	{
		...
		if (bCreateData)
		{
			...
			struct FNewPart
			{
				...
				FIntVector SummedPositions = FIntVector::ZeroValue;
			...
			{
				NewPart.SummedPositions += Position;
				NewPart.NumPositions++;
			...
			{
				OutResult.Parts.Add({ NewPart.NewPartCenter, NewPart.NewPartData, {}, FVector(NewPart.SummedPositions) / NewPart.NumPositions });
			};
	...
	FVoxelRemoveFloatingPartsResult&& RemoveFloatingPartsResult)
{
	...
	for (FVoxelFloatingPart& NewPart : RemoveFloatingPartsResult.Parts)
	{
		TVoxelSharedPtr<FSimpleDelegate> OnWorldUpdateDone;
		const auto Result = PartSpawner.SpawnPart(OnWorldUpdateDone, World, MoveTemp(NewPart));
		if (Result)
		{
			Results.Add(Result);
		}
	...
	{
		World->GetSubsystemChecked<IVoxelLODManager>().UpdateBounds_OnAllFinished(RemoveFloatingPartsResult.BoxToUpdate.GetBox(), FSimpleDelegate::CreateLambda(Finish));
	}

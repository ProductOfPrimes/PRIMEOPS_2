...
void UVoxelBlueprintLibrary::RegenerateSpawners(AVoxelWorld* World, FVoxelIntBox Bounds)
{
	...
	World->GetSubsystemChecked<IVoxelFoliageInterface>().Regenerate(Bounds);
}
...
void UVoxelBlueprintLibrary::MarkSpawnersDirty(AVoxelWorld* World, FVoxelIntBox Bounds)
{
	...
	World->GetSubsystemChecked<IVoxelFoliageInterface>().MarkDirty(Bounds);
}
...
bool UVoxelBlueprintLibrary::IsVoxelWorldFoliageLoading(AVoxelWorld* World)
{
	...
	const auto PoolCounters = GVoxelThreadPool->GetCountersForPool(PoolId);
	return
		PoolCounters.GetNumTasksForType(EVoxelTaskType::FoliageBuild) > 0 ||
		PoolCounters.GetNumTasksForType(EVoxelTaskType::HISMBuild) > 0;
}

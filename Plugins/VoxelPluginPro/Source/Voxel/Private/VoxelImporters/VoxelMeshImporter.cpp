	...
	int32& OutNumLeaks)
{
	...
	if (Settings.bImportColors)
	{
		...
		if (!RenderTargetCache.ColorsRenderTarget || Settings.ColorsMaterial != RenderTargetCache.LastRenderedColorsMaterial)
		{
			...
			FVoxelTextureHelpers::ClearCache(RenderTargetCache.ColorsRenderTarget);
		...
		ColorTexture = FVoxelTextureHelpers::CreateFromTexture_Color(RenderTargetCache.ColorsRenderTarget);
	}
	if (Settings.bImportUVs)
	{
		...
		if (!RenderTargetCache.UVsRenderTarget || Settings.UVsMaterial != RenderTargetCache.LastRenderedUVsMaterial)
		{
			...
			FVoxelTextureHelpers::ClearCache(RenderTargetCache.UVsRenderTarget);
		...
		UVTexture = FVoxelTextureHelpers::CreateFromTexture_Color(RenderTargetCache.UVsRenderTarget);
	}

...
void FVoxelData::GetDiffs(TArray<TVoxelChunkDiff<FVoxelValue>>& OutValueDiffQueue, TArray<TVoxelChunkDiff<FVoxelMaterial>>& OutMaterialDiffQueue)
{
	VOXEL_ASYNC_FUNCTION_COUNTER();
	
	FVoxelReadScopeLock Lock(*this, FVoxelIntBox::Infinite, FUNCTION_FNAME);
	FVoxelOctreeUtilities::IterateAllLeaves(GetOctree(), [&](FVoxelDataOctreeLeaf& Leaf)
	{
		// TODO: array of dirty chunks instead of whole octree iteration
		ensureThreadSafe(Leaf.IsLockedForRead());
		if (Leaf.Multiplayer.IsValid())
		{
			if (Leaf.Multiplayer->IsNetworkDirty<FVoxelValue>())
			{
				auto& Diff = OutValueDiffQueue.Emplace_GetRef(Leaf.Position);
				Leaf.Multiplayer->AddToDiffQueueAndReset(Leaf.Values, Diff.Diffs);
			}
			if (Leaf.Multiplayer->IsNetworkDirty<FVoxelMaterial>())
			{
				auto& Diff = OutMaterialDiffQueue.Emplace_GetRef(Leaf.Position);
				Leaf.Multiplayer->AddToDiffQueueAndReset(Leaf.Materials, Diff.Diffs);
			}
		}
	});
}
...
template<typename T>
inline void LoadFromDiffsImpl(FVoxelData& Data, TArray<TVoxelChunkDiff<T>>& Queue, TArray<FVoxelIntBox>& OutModifiedBounds)
{
	VOXEL_ASYNC_FUNCTION_COUNTER();

	for (auto& ChunkDiff : Queue)
	{
		auto Bounds = FVoxelIntBox(ChunkDiff.Position - FIntVector(DATA_CHUNK_SIZE / 2), ChunkDiff.Position + FIntVector(DATA_CHUNK_SIZE / 2));
		FVoxelWriteScopeLock Lock(Data, Bounds, FUNCTION_FNAME);

		FVoxelDataOctreeLeaf& Leaf = 
			*FVoxelOctreeUtilities::GetLeaf<EVoxelOctreeLeafQuery::CreateIfNull>(
				Data.GetOctree(),
				ChunkDiff.Position);

		ensureThreadSafe(Leaf.IsLockedForWrite());
		check(Leaf.Position == ChunkDiff.Position);
		check(ChunkDiff.Diffs.Num() > 0);
		...
		{
			checkVoxelSlow(0 <= Diff.Index && Diff.Index < VOXELS_PER_DATA_CHUNK);
			DataHolder.Set(Diff.Index, Diff.Value);
		}

		OutModifiedBounds.Emplace(Leaf.GetBounds());
	}
}

void FVoxelData::LoadFromDiffs(TArray<TVoxelChunkDiff<FVoxelValue>>& ValueDiffQueue, TArray<TVoxelChunkDiff<FVoxelMaterial>>& MaterialDiffQueue, TArray<FVoxelIntBox>& OutModifiedBounds)
{
	VOXEL_ASYNC_FUNCTION_COUNTER();
	
	LoadFromDiffsImpl(*this, ValueDiffQueue, OutModifiedBounds);
	LoadFromDiffsImpl(*this, MaterialDiffQueue, OutModifiedBounds);
}

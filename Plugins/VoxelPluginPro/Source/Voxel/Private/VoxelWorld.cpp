...
AVoxelWorld::AVoxelWorld()
{
	...
	{
		if (!bAutomaticallyRefreshFoliage)
		{
			return;
		}

		const TVoxelSharedPtr<IVoxelFoliageInterface> FoliageSubsystem = GetSubsystem<IVoxelFoliageInterface>();
		if (FoliageSubsystem && FoliageSubsystem->NeedsToRebuild(Object, PropertyChangedEvent))
		{
			bRegenerateFoliageOnNextFrame = true;
		}
	};

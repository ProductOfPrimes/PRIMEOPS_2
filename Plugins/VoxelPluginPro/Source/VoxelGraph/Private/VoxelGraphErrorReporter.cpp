	...
	bool bShowInList)
{
	...
	{
		AddMessageToNode(SourceNode, Message, Severity, bSelectNode, bShowInList);
	}
}
	...
	bool bShowInList)
{
	...
	for (auto& SourceNode : Node->SourceNodes)
	{
		...
		{
			AddMessageToNode(SourceNode.Get(), Message, Severity, bSelectNode, bShowInList);
		}
	}
}
...
bool FEnsureVoxelGraphHelper::Check(FVoxelGraphErrorReporter& ErrorReporter, const FString& Expr, const FString& File, int32 Line, const FVoxelCompilationNode* Node)
{
	...
	if (Node)
	{
		ErrorReporter.AddMessageToNode(Node, Message, EVoxelGraphNodeMessageType::Error);
	}
	return true;
}

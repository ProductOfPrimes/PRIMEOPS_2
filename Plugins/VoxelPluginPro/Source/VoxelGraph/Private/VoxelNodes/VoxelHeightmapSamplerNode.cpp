...
public:
	...
	FVoxelNode_HeightmapSampler_LocalVoxelComputeNode(const UVoxelNode_HeightmapSampler& Node, const FVoxelCompilationNode& CompilationNode)
		...
		, DataUINT16(bFloat ? nullptr : Node.GetParameter(CompilationNode.Context))
		, DataFloat(bFloat ? Node.GetParameter(CompilationNode.Context) : nullptr)
		, Variable(bFloat
...
UVoxelHeightmapAsset* UVoxelNode_HeightmapSampler::GetParameter(const FVoxelGraphContext& Context) const
{
	return GetParameterInternal<UVoxelHeightmapAsset>(Context);
}
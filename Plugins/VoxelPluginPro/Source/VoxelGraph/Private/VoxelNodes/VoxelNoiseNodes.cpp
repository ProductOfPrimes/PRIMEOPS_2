...
TSharedPtr<FVoxelCompilationNode> UVoxelNode_NoiseNode::GetCompilationNode(const FVoxelGraphContext& Context) const
{
	if (NeedRangeAnalysis())
	{
		// We handle range analysis ourselves, without using the inputs
		return MakeShared<FVoxelRangeAnalysisConstantCompilationNode>(Context, *this);
	}

	// eg for gradient perturb
	return Super::GetCompilationNode(Context);
}
